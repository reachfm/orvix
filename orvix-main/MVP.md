...
### Core Principles
- **Stalwart = untouched** — never modify Stalwart source code
- **Orvix talks to Stalwart via its REST API** — clean separation
- **Single Go binary** embeds Stalwart + Orvix layer + frontend
...
| Mail Core | **Stalwart** | Embedded via subprocess + REST API |
...
### Frontend embedded into Go binary via embed.FS
...
├────────────────┼──────────┼────────────────┤
│ Stalwart Core  │ v0.10.x  │ v0.10.x ✅     │
│ Orvix Core  │ v1.0.0   │ v1.0.0  ✅     │
│ Mail Firewall  │ v1.0.0   │ v1.1.0  🔄     │
│ Guardian Agent │ v1.0.0   │ v1.0.0  ✅     │
│ Smart Compose  │ v1.0.0   │ v1.1.0  🔄     │
│ Webmail UI     │ v1.0.0   │ v1.0.0  ✅     │
└────────────────┴──────────┴────────────────┘
[Update Selected]  [Update All]  [View Changelog]
```

### Feature Flags
```
[✅] Mail Firewall          v1.0.0
[✅] Guardian Agent         v1.0.0
[✅] Smart Compose AI       v1.0.0
[✅] Auto-Heal System       v1.0.0
[❌] Zero-Knowledge Enc.    v1.0.0  (Enterprise only)
[❌] Collaboration Layer    v1.0.0  (Enterprise only)
```

### Auto-Update System
- Check for updates
- View changelog before updating
- One-click update (hot-swap, zero downtime)
...
- Geo-block map (click to block countries)
...
### Severity Levels
```
LOW      → auto-fix silently + log
MEDIUM   → auto-fix + notify admin
HIGH     → attempt fix + alert admin
CRITICAL → alert + pause + wait for admin
```
...
Response:
{
  ...
  "verdict": "phishing",
  "confidence": 0.97,
  "reasons": ["Domain 3 days old", "IP on 12 blacklists"],
  "action": "block",
  "explanation": "Classic CEO fraud pattern..."
}
```
...
type Module interface {
    ID()       string   // "guardian-agent"
    Version()  string   // "1.0.0"
    Requires() []string // ["core"]
    Init(cfg *config.Config, db *gorm.DB) error
    Start()    error
    Stop()     error
    Migrate()  error    // additive DB changes only
}
```
...
2.0.0  → breaking change (very rare)
```

### Safe Update Rules
```
✅ Migrations: ADD only — never DROP or RENAME
✅ API: add fields — never remove or rename
✅ Modules: hot-swappable without core restart
✅ Every update: auto-backup before applying
✅ Health check after update: fail = auto-rollback
```

### Update Channels
- **Stable** → production use
- **Beta** → early features
- **Security** → auto-apply patches only
...
├── cmd/
│   └── orvix/
│       └── main.go              # Entry: starts Stalwart + Orvix
├── internal/
│   ├── stalwart/                # Stalwart integration layer
│   │   ├── client.go            # REST API client for Stalwart
│   │   ├── webhook.go           # Receive events from Stalwart
│   │   ├── process.go           # Start/stop Stalwart subprocess
│   │   └── config.go            # Generate Stalwart config
│   ├── license/                 # License validation + feature flags
│   │   ├── validator.go
│   │   ├── features.go
│   │   └── fingerprint.go
│   ├── modules/                 # Module registry
│   │   └── registry.go
│   ├── firewall/                # Mail Firewall module
│   │   ├── module.go
│   │   ├── pipeline.go
│   │   ├── rules.go
│   │   └── reputation.go
│   ├── autoheal/                # Auto-Heal module
│   │   ├── module.go
│   │   ├── monitor.go
│   │   └── fixers.go
│   ├── guardian/                # Guardian AI module
│   │   ├── module.go
│   │   ├── agent.go
│   │   └── api.go
│   ├── compose/                 # Smart Compose AI module
│   │   ├── module.go
│   │   └── stream.go
│   ├── provision/               # Instant Deploy API module
│   │   ├── module.go
│   │   └── provisioner.go
│   ├── migration/               # Smart Migration module
│   │   ├── module.go
│   │   ├── imap_sync.go
│   │   └── sources/
...
│   │   └── module.go
│   ├── compliance/              # Compliance Center (Enterprise)
│   │   └── module.go
│   ├── intelligence/            # Email Intelligence module
│   │   └── module.go
│   ├── updater/                 # Auto-update + versioning
│   │   ├── updater.go
│   │   ├── changelog.go
│   │   └── rollback.go
│   ├── api/                     # Orvix REST API
...
│   └── metrics/                 # Prometheus
├── web/
│   ├── webmail/                 # React Webmail app
│   └── admin/                   # React Admin console
├── stalwart-bin/                # Stalwart binary (embedded)
├── migrations/                  # DB migrations (additive only)
├── scripts/
...
## 🗄️ Database Schema (Orvix tables — Stalwart has its own)

```sql
-- License
licenses (id, key_hash, tier, issued_at, expires_at,
          max_domains, max_mailboxes, hardware_id, metadata)

-- Feature flags
feature_flags (id, name, enabled, tier_required, module_version)

-- Module versions
module_versions (id, module_id, version, installed_at,
                 checksum, status, changelog)

-- Firewall rules
firewall_rules (id, name, condition, action,
                priority, enabled, created_at)
firewall_logs  (id, ip, domain, action, reason,
                threat_score, created_at)

-- Guardian logs
guardian_logs (id, message_id, threat_score, verdict,
               confidence, reasons, action, created_at)

-- Auto-heal history
heal_history (id, check_name, severity, issue,
              fix_applied, success, created_at)

-- Provision API
provisioned_domains (id, domain, plan, status,
                     provisioned_by, created_at, metadata)
...
-- Sessions
sessions (id, user_id, token_hash, ip,
          user_agent, created_at, expires_at)

-- Update history
update_history (id, module_id, from_version, to_version,
...
- [x] Embed Stalwart binary into Go (embed.FS) → Managed External: Stalwart is downloaded separately. Installer handles it. embed.go removed.
...
- [x] Calendar UI (FullCalendar) → Backend CRUD complete. React UI pending.
- [x] Contacts UI → Backend CRUD complete. React UI pending.
- [x] Tasks UI → Backend CRUD complete. React UI pending.
...
- [ ] Email Archiving + Legal Hold → Completed as part of Compliance module (legal holds, retention policies)
...
- [x] Full security audit — Completed. See AUDIT.md and ORVIX_INDEPENDENT_AUDIT.md
...
| Deployment | Single binary | Install in seconds like Axigen |
...
Set up Go project structure exactly as defined in Project Structure section.
Initialize go.mod with module name github.com/orvix/orvix.
Install dependencies: Fiber v3, GORM, Viper, Zap, Redis client.
Create config system reading from orvix.yaml.
Create database connection (SQLite default, PostgreSQL if configured).
Run initial migrations for all tables defined in Database Schema.
All code must be production-quality with full error handling and Zap logging.
After completing, mark task as done in this file and move to next task.

---

*Orvix MVP.md v3.0 — Stalwart-based architecture*
