...
export async function renderDnsDkimPage(root) {
  ...
  const mtaBody = el('div', { class: 'panel-body' });
...
function renderMtaStsCard(host) {
  ...
  const policy = [
    'version: STSv1',
    'mode: testing',
    'mx: mail.' + (state.currentDomain || 'example.com'),
    'max_age: 86400',
  ].join('\n');
  const txt = 'v=STSv1; id=' + new Date().toISOString().slice(0, 10).replace(/-/g, '');
  host.appendChild(el('dl', { class: 'kv' }, [
    ...
    el('dd', null, el('pre', { class: 'code', text: policy })),
  ]));
}
...
function renderDnsProviderPanel(host, domain) {
  host.innerHTML = '';
  const providers = [
    { name: 'manual',  status: 'ready',          plan_supported: true,  plan_needed: true  },
    { name: 'cloudflare', status: 'not_configured', plan_supported: false, plan_needed: false },
    { name: 'namecheap',  status: 'not_configured', plan_supported: false, plan_needed: false },
  ];
  const status = (p) => {
  ...
  const tbl = table({
    columns: [
      { name: 'p', label: 'Provider', render: (p) => p.name === 'manual' ? 'manual' : p.name },
  ...
  // If a plan is already stored in state for this domain, render it.
  if (domain && state.dnsProviderPlans[domain]) {
    host.appendChild(renderChangePlan(state.dnsProviderPlans[domain]));
  }

  // Apply-disabled state: when there are conflicts OR no providers
  // are ready, the Apply button is disabled.
  state.applyDisabled = !(providers.find((p) => p.name === 'manual'));
}
