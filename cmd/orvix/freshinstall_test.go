...
func TestFreshInstall_DatabaseHasExactlyOneAdminUser(t *testing.T) {
	...
	// PORTAL-SEPARATION-PHASE1: role set MUST match release/install.sh
	// verify_install / active_admin_count / admin_user_exists. Drift
	// between install.sh and this test fixture is caught by
	// TestInstallerAdminPredicatesRecognizeCanonicalRole; keep both in
	// lockstep whenever the canonical role set changes.
	var count int
	if err := h.sqlDB.QueryRow(
		`SELECT COUNT(*) FROM users WHERE email = ? AND role IN ('admin','superadmin','super_admin','platform_super_admin') AND active = 1`,
		h.email,
	).Scan(&count); err != nil {
