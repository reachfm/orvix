...
| `GET /platform/support/tickets` | platformMW | `ListPlatformSupportTickets` | paginated, filtered list of every support ticket across every tenant (FINAL-ENTERPRISE-COMPLETION) | Platform | `internal/support/` | MISSING_UI | UI_SUPPORTED (features/platform/support-inbox/page.tsx; parity matrix 5.2a COMPLETE) |
| `GET /platform/support/tickets/:ref` | platformMW | `GetPlatformSupportTicket` | read any ticket detail + reply thread | Platform | `internal/support/` | MISSING_UI | UI_SUPPORTED (features/platform/support-inbox/page.tsx ticket detail) |
| `POST /platform/support/tickets/:ref/reply` | platformMW | `ReplyOnPlatformSupportTicket` | platform operator reply on any ticket (drives canonical status transition) | Platform | `internal/support/` | MISSING_UI | UI_SUPPORTED (features/platform/support-inbox/page.tsx reply) |
| `POST /platform/support/tickets/:ref/status` | platformMW | `UpdatePlatformSupportTicketStatus` | guarded status transition (canonical graph; closed is terminal) | Platform | `internal/support/` | MISSING_UI | UI_SUPPORTED (features/platform/support-inbox/page.tsx status change) |
