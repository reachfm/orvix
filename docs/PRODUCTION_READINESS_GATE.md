...
| 4 | **Release blocker** | uninstall path | `uninstall.sh` ran `userdel -r orvix` without further confirmation — that deleted `/var/lib/orvix` (the entire mailstore + DB + backups). The script also referenced a stale legacy mail-engine binary that hasn't shipped since RC3. | Added `--purge-data` flag requiring a typed confirmation phrase. Default install is now non-destructive (preserves data + copies to backup dir). Dropped the legacy mail-engine references. |
...
| 6 | Medium | legacy cruft | `healthcheck.sh` and `diagnostics.sh` referenced a legacy third-party mail server that hasn't shipped since RC3 — would mislead any future triager. | Dropped all legacy mail-engine references. Added JMAP listener probe to `healthcheck.sh` and extended the port list in `diagnostics.sh`. |
