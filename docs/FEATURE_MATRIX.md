...
| Queue attempt detail | `internal/api/handlers/admin_queue.go` | admin queue.js | `/admin/queue/:id` | **`queue_attempts` — table missing** | n/a | **No** | High | BLOCKED |
...
| Admin console (Vite SPA) | N/A | `web/admin` (38 files) | `/api/v1/admin/*` (~180 endpoints) | — | — | Yes, with 1 confirmed routing bug | Critical | PARTIAL |
...
| Customer — groups | customer_mail.go | web/admin | `/enterprise/groups*` | **`coremail_groups`/`coremail_group_members` — tables missing** | 12 tests (test-local schema only) | **No — non-functional in every environment** | High | BLOCKED |
...
| IDOR protections (tenant isolation) | `internal/api/handlers/*` | — | scattered across `/enterprise` and `/admin` | — | `tenant_isolation_test.go` (mailboxes/domains) + new `customer_mail_tenant_isolation_test.go` (aliases/groups) | Mostly yes; `requireTenantActive` itself is broken (missing `organizations` table) | Critical | PARTIAL |
