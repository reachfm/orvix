...
| IDOR protections (tenant isolation) | `internal/api/handlers/*` | — | scattered across `/enterprise` and `/admin` | — | `tenant_isolation_test.go` (mailboxes/domains) + `customer_mail_tenant_isolation_test.go` (aliases/groups) + `enterprise_mutation_smoke_test.go` (full-router `requireTenantActive` coverage) | Yes — `requireTenantActive` fixed this session (was querying nonexistent `organizations` table and panicking on a GORM `.Raw().Scan()` bug) | Critical | COMPLETE |
