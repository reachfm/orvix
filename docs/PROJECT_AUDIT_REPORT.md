...
| Security | 7 (unchanged) | RBAC + CSRF + tenant-isolation are real and increasingly well-tested. Confirmed IDOR classes found *and fixed* in this codebase's history: mailbox/domain read, alias/group, `requireTenantActive`, and now the mailbox/domain **CSV export** IDOR (fixed + 10 router-level regression tests this pass). Score held (not raised) because the export fix *also surfaced* an equivalent still-open defect — `ListDomains` (`GET /api/v1/domains`) is unscoped by tenant, same class — plus thin DKIM/SPF/DMARC test depth remains. One gap closed, one comparable gap now known-open and documented. |
...
7. **`ListDomains` (`GET /api/v1/domains`) has no tenant scoping** — same missing-tenant-scope defect class as the just-fixed exports, surfaced while fixing them. A tenant admin sees every tenant's domains. *(new, open — list handler, deliberately out of the export-security pass scope; see `docs/MASTER_TODO.md`)*
...
5. **`ListDomains` tenant-scoping gap** — a tenant admin can list every tenant's domains via `GET /api/v1/domains`. *(new, open — see Technical Debt #7)*
...
5. ~~Fix the `ExportMailboxesCSV` tenant-scoping gap~~ — **DONE** (mailbox + domain CSV export, `mailbox_export_isolation_test.go`). Next in this class: fix the `ListDomains` list-handler tenant-scoping gap surfaced by this pass, and sweep sibling admin-group list handlers.
