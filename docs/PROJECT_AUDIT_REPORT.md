...
| Security | 7 (was 6) | RBAC + CSRF + tenant-isolation are real and tested (two confirmed IDOR classes found *and fixed* in this codebase's history — mailbox/domain, then alias/group). The `requireTenantActive` bug (queried a nonexistent table, plus a co-located GORM panic) is now **fixed and tested** this session. Still docked from a perfect score for the newly-discovered `ExportMailboxesCSV` tenant-scoping gap (not yet fixed) and thin DKIM/SPF/DMARC test depth. |
...
6. **`ExportMailboxesCSV` has no tenant scoping** — newly discovered this session while building the correctly-scoped `ListMailboxes`. *(new, open)*
...
4. **`ExportMailboxesCSV` tenant-scoping gap** — a tenant admin can export every tenant's mailbox list via CSV. *(new, open — see Technical Debt #6)*
...
5. Fix the newly-discovered `ExportMailboxesCSV` tenant-scoping gap. *(next up — small, well-scoped)*
...
**~78% enterprise-ready (up from ~72%).** Core mail engine, security primitives (RBAC/CSRF/tenant-isolation), billing, backup/restore, monitoring, DNS ops, and — as of this session — the customer Groups feature and the `/enterprise` mutation gate are all COMPLETE and tested. The remaining ~22% is concentrated in: a handful of genuinely unimplemented enterprise-tier features (SSO, WebAuthn, reseller portal), the newly-discovered `ExportMailboxesCSV` scoping gap, and maintainability debt (handler sprawl, licensing-package overlap) that doesn't block shipping but will slow every future change until addressed.
