...
| Maintainability | 6.5 (unchanged) | 0.70 test-files-per-source-file is a reasonable breadth signal. The five confirmed schema/routing defects from the initial audit pass are fixed and covered by regression tests, demonstrating the fix-and-test cycle works. This pass also stabilized a flaky panic in `TestRehashOnLogin_ConcurrentPasswordChangeNotOverwritten` (test-harness timing bug, not production) — but found an identical unfixed sibling in `tenant_isolation_matrix_test.go`, so the score is held rather than raised: the underlying pattern (test helpers omitting Fiber's `Test()` timeout override) is not yet eliminated package-wide. |
...
6. ~~`TestRehashOnLogin_ConcurrentPasswordChangeNotOverwritten` flaky panic~~ — **FIXED**: was a test-harness timing bug (missing Fiber `Test()` timeout override), not a production race; confirmed by source inspection (no goroutines/channels in the test) and 50/50 + 120/120 repeated-run stability after the fix. **Sibling instance found, not fixed:** `tenant_isolation_matrix_test.go`'s `matrixLogin`/`matrixCSRF` have the identical missing-timeout pattern — flagged in `docs/MASTER_TODO.md` for a follow-up, out of scope for this pass.
