  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="flex items-center gap-1 bg-[var(--accent)] text-white rounded px-4 py-2 text-sm">
      ...
      {showCreate && (
        <div className="bg-[var(--bg-elevated)] border border-[var(--bg-subtle)] rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            {create.error && <p className="text-[var(--danger)] text-sm">{create.error.message}</p>}
          </div>
        </div>
      )}

      <div className="bg-[var(--bg-elevated)] border border-[var(--bg-subtle)] rounded-lg">
        <div className="p-4 border-b border-[var(--bg-subtle)]">
        ...
        <div className="divide-y divide-[var(--bg-subtle)]">
          {Array.isArray(list) && list.filter((m: any) => !search || m.email?.includes(search)).map((m: any) => (
            <div key={m.id} className="flex items-center justify-between p-4 hover:bg-[var(--bg-elevated)]">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs px-2 py-0.5 rounded ${m.status === "active" ? "bg-[var(--success)]/10 text-[var(--success)]" : "bg-[var(--warning)]/10 text-[var(--warning)]"}`}>
                  {m.status}
                </span>
              </div>
