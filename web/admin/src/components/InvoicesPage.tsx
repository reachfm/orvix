...
export default function InvoicesPage() {
  ...
  const statusColors: Record<string, string> = {
    active: "bg-[var(--status-success)]/10 text-[var(--status-success)]",
    trialing: "bg-[var(--accent-blue)]/10 text-[var(--accent-blue)]",
    past_due: "bg-[var(--accent-yellow)]/10 text-[var(--accent-yellow)]",
    cancelled: "bg-[var(--status-danger)]/10 text-[var(--status-danger)]",
    suspended: "bg-[var(--status-danger)]/10 text-[var(--status-danger)]",
    paid: "bg-[var(--status-success)]/10 text-[var(--status-success)]",
    open: "bg-[var(--accent-blue)]/10 text-[var(--accent-blue)]",
    void: "bg-[#555D73]/10 text-[var(--text-muted)]",
    uncollectible: "bg-[var(--status-danger)]/10 text-[var(--status-danger)]",
  };
      ...
      {sub ? (
        <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            {sub.status && (
              <span className={`px-2 py-1 rounded text-xs font-medium ${statusColors[sub.status] || "text-[var(--text-secondary)] bg-gray-400/10"}`}>
                {sub.status}
              </span>
            )}
          </div>
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[var(--bg-base)] rounded-lg p-4">
              <Mail className="w-4 h-4 text-[var(--accent-blue)] mb-2" />
            ...
            <div className="bg-[var(--bg-base)] rounded-lg p-4">
              <CreditCard className="w-4 h-4 text-[var(--status-success)] mb-2" />
            ...
            <div className="bg-[var(--bg-base)] rounded-lg p-4">
              <HardDrive className="w-4 h-4 text-[var(--accent-yellow)] mb-2" />
            ...
            <div className="bg-[var(--bg-base)] rounded-lg p-4">
              <HardDrive className="w-4 h-4 text-[var(--status-danger)] mb-2" />
        ...
        {invList.length > 0 ? (
          <div className="overflow-hidden rounded-lg border border-[var(--border)]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {invList.map((inv: any) => (
                  <tr key={inv.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-shell)]">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${statusColors[inv.status] || "text-[var(--text-secondary)] bg-gray-400/10"}`}>
                        {inv.status}
                      </span>
                    </td>
