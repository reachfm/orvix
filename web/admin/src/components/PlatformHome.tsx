...
export default function PlatformHome({
  ...
  return (
    <div>
      ...
      {dashQ.isLoading ? (
        <div className="flex items-center gap-2 text-[#8B92A8] mb-8"><Loader2 size={16} className="animate-spin" /> Loading platform totals…</div>
      ) : dashQ.error ? (
        <div className="flex items-center gap-2 text-[#F87171] text-sm mb-8"><AlertCircle size={16} /> Failed to load platform dashboard: {(dashQ.error as Error).message}</div>
      ) : dashQ.data ? (
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-[#13161C] border border-[#2A2F3E] rounded-xl p-4">
            <p className="text-xs text-[#8B92A8] mb-1">Tenants</p>
            <p className="text-2xl font-bold text-[#E8EAF0]">{dashQ.data.tenants?.total ?? "—"}</p>
            <p className="text-xs text-[#555D73] mt-1">{dashQ.data.tenants?.active ?? 0} active</p>
          </div>
          <div className="bg-[#13161C] border border-[#2A2F3E] rounded-xl p-4">
            <p className="text-xs text-[#8B92A8] mb-1">Mailboxes</p>
            <p className="text-2xl font-bold text-[#E8EAF0]">{dashQ.data.mailboxes?.total ?? "—"}</p>
          </div>
          <div className="bg-[#13161C] border border-[#2A2F3E] rounded-xl p-4">
            <p className="text-xs text-[#8B92A8] mb-1">Domains</p>
            <p className="text-2xl font-bold text-[#E8EAF0]">{dashQ.data.domains?.total ?? "—"}</p>
          </div>
          <div className="bg-[#13161C] border border-[#2A2F3E] rounded-xl p-4">
            <p className="text-xs text-[#8B92A8] mb-1">Queue</p>
            <p className={`text-2xl font-bold ${(dashQ.data.queue?.failed ?? 0) > 0 ? "text-[#F87171]" : "text-[#E8EAF0]"}`}>{dashQ.data.queue?.pending ?? 0}</p>
            <p className="text-xs text-[#555D73] mt-1">{dashQ.data.queue?.failed ?? 0} failed</p>
          </div>
        </div>
      ) : null}
