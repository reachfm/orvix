...
export default function AliasesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--bg-elevated)] border border-[var(--bg-subtle)] rounded-lg divide-y divide-[var(--bg-subtle)]">
        {aliases && aliases.length > 0 ? aliases.map((a: any) => (
          <div key={a.id} className="flex items-center justify-between p-4 hover:bg-[var(--bg-elevated)]">
            <div className="flex items-center gap-3">
              <AtSign className="w-4 h-4 text-[var(--text-secondary)]" />
              <span className="text-[var(--text-primary)] text-sm">{a.alias}</span>
              <span className="text-[var(--text-muted)] text-sm">→ {a.target_email}</span>
            </div>
            <button onClick={() => remove.mutate(a.id)} className="text-[var(--text-secondary)] hover:text-[var(--danger)]">
