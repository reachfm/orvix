    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {groups && groups.length > 0 ? groups.map((g: any) => (
          <div key={g.id} className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg">
            <button onClick={() => setExpandedGroup(expandedGroup === g.id ? null : g.id)}
              className="w-full flex items-center justify-between p-4 hover:bg-[var(--bg-shell)]">
              ...
              <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => remove.mutate(g.id)} className="text-[var(--text-secondary)] hover:text-red-400">
              ...
              <div className="px-4 pb-4 border-t border-[var(--border)] pt-3">
                <div className="flex gap-2 mb-3">
                  ...
                  <button onClick={() => addMember.mutate({ groupId: g.id, email: newMember })}
                    disabled={!newMember} className="text-[var(--accent-blue)] hover:text-[#3D6AE8]">
                ...
                {g.members?.map((m: any) => (
                  <div key={m.id || m.email} className="flex items-center justify-between py-1.5 text-sm">
                    ...
                    <button onClick={() => removeMember.mutate({ groupId: g.id, memberId: m.id })} className="text-[var(--text-secondary)] hover:text-red-400">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ))}
          ...
          <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-6 text-center text-[var(--text-secondary)] text-sm">
