...
function DNSHealthDrawer({ domain, onClose }: { domain: EnterpriseDomain; onClose: () => void }) {
  ...
  // Escape closes the drawer, and Tab is trapped inside it while open —
  // without this, Tab/Shift+Tab can move focus onto elements behind the
  // modal overlay, which is unreachable/invisible to a keyboard user.
  useEffect(() => {
  ...
  // Move focus into the drawer on open, and restore it to whatever
  // triggered the drawer (the domain row) when the drawer unmounts —
  // otherwise focus silently resets to document.body on close.
  useEffect(() => {
