    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-medium text-[var(--text-primary)]">Profile</h3>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-sm text-[var(--text-secondary)] mb-1">Email</label>
            <input value={profile?.email || ""} disabled
              className="w-full px-3 py-2 bg-[var(--bg-base)] border border-[var(--border)] rounded text-[var(--text-muted)] text-sm" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm text-[var(--text-secondary)] mb-1">Locale</label>
          ...
          {updateProfile.error && (
            <p className="text-[var(--status-danger)] text-sm">{(updateProfile.error as any)?.message || "Failed to update profile"}</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-6">
        <div className="flex items-center gap-3 mb-4">
          <Lock className="w-5 h-5 text-[var(--accent-blue)]" />
        ...
        <div className="space-y-3">
          ...
          {changePassword.isSuccess && <p className="text-[var(--status-success)] text-sm">Password updated.</p>}
          {changePassword.error && <p className="text-[var(--status-danger)] text-sm">{(changePassword.error as any)?.message || "Failed to update password"}</p>}
        </div>
      </div>
    </div>
  );
