...
// Repository is the incident persistence layer.
...
func (r *Repository) EnsureSchema(ctx context.Context) error {
	...
	_, err := r.db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS platform_incidents (
		id `+autoInc+`,
		title TEXT NOT NULL,
...
func (r *Repository) Insert(ctx context.Context, inc *Incident) error {
	srvJSON, _ := json.Marshal(inc.Services)
	regJSON, _ := json.Marshal(inc.Regions)
	tidJSON, _ := json.Marshal(inc.TenantIDs)
	now := time.Now().UTC()
	res, err := r.db.ExecContext(ctx,
		`INSERT INTO platform_incidents (title, description, severity, status, services, regions, tenant_ids, version, created_at, updated_at)
		VALUES (`+r.dialect.Placeholders(10)+`)`,
		inc.Title, inc.Description, inc.Severity, StatusInvestigating, string(srvJSON), string(regJSON), string(tidJSON), 1, now, now)
	if err != nil {
		return err
	}
...
func (r *Repository) Update(ctx context.Context, inc *Incident) error {
	...
	res, err := r.db.ExecContext(ctx,
		...
		inc.Title, inc.Description, inc.Severity, inc.Status, string(srvJSON), string(regJSON), string(tidJSON), now, inc.ResolvedAt, inc.ID, inc.version)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("concurrent modification: incident may have been updated")
	}
	inc.UpdatedAt = now
	inc.version++
	return nil
...
func (r *Repository) AddTimelineEvent(ctx context.Context, ev *TimelineEvent) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO platform_incident_events (incident_id, status, message, operator, created_at) VALUES (?,?,?,?,?)`,
...
func (r *Repository) PublicStatus(ctx context.Context) (*PublicStatus, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, title, severity, status, created_at, updated_at FROM platform_incidents WHERE status IN ('investigating','identified','monitoring') ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	// Maintenance
	maintRows, err := r.db.QueryContext(ctx,
		`SELECT id, title, severity, status, created_at, updated_at FROM platform_incidents WHERE severity=? ORDER BY created_at DESC`, SevScheduled)
	if err == nil {
...
func (r *Repository) scanIncident(row *sql.Row) (*Incident, error) {
	...
	var srvJSON, regJSON, tidJSON string
	var resolvedAt sql.NullTime
	if err := row.Scan(&inc.ID, &inc.Title, &inc.Description, &inc.Severity, &inc.Status, &srvJSON, &regJSON, &tidJSON, &inc.version, &inc.CreatedAt, &inc.UpdatedAt, &resolvedAt); err != nil {
		return nil, err
	}
	json.Unmarshal([]byte(srvJSON), &inc.Services)
	json.Unmarshal([]byte(regJSON), &inc.Regions)
