...
func TestGetJob(t *testing.T) {
	...
	if got.SourceType != ImportIMAP {
		t.Fatalf("expected imap, got %s", got.SourceType)
	}
}
...
func TestCancelJob(t *testing.T) {
	...
	if err := svc.CancelJob(ctx, j.ID); err != nil {
		t.Fatalf("cancel: %v", err)
	}
	got, _ := svc.GetJob(ctx, j.ID)
	if got.Status != ImpCancelled {
...
func TestImportDomain(t *testing.T) {
	...
	got, _ := svc.GetJob(ctx, j.ID)
	if got.DomainsImported != 1 {
		t.Fatalf("expected 1 domain, got %d", got.DomainsImported)
	}
}
...
func TestImportDuplicateDomain(t *testing.T) {
	...
	got, _ := svc.GetJob(ctx, j.ID)
	if got.Errors != 1 {
		t.Fatalf("expected 1 error for duplicate, got %d", got.Errors)
	}
}
...
func TestImportMailbox(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("import mailbox: %v", err)
	}

	got, _ := svc.GetJob(ctx, j.ID)
	if got.MailboxesImported != 1 {
		t.Fatalf("expected 1 mailbox, got %d", got.MailboxesImported)
	}
}
...
func TestImportMessage(t *testing.T) {
	...
	got, _ := svc.GetJob(ctx, j.ID)
	if got.MessagesImported != 1 {
		t.Fatalf("expected 1 message, got %d", got.MessagesImported)
	}
}
...
func TestMultipleSources(t *testing.T) {
	...
	for _, s := range sources {
		...
		if j.SourceType != s {
			t.Fatalf("expected %s, got %s", s, j.SourceType)
		}
	}
}
